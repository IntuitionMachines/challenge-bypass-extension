...
const respBadProof = `signatures=
...
each(PPConfigs().filter((config) => config.id > 0).map((config) => [config.id]))
    .describe("configId = %i signing request is cancelled", (configId) => {
        test("signing off", () => {
...
each(PPConfigs().filter((config) => config.id > 0).map((config) => [config.id]))
    .describe("configId = %i, test sending sign requests", (configId) => {
    ...
    describe("test validation and storage", () => {
        ...
        test("test store tokens", () => {
            ...
            function run() {
                ...
                before = getMock(bypassTokensCount(configId));
                xhr.onreadystatechange();
                after = getMock(bypassTokensCount(configId));
                version = out.version;
            }
            setTimeSinceLastResp(0); // reset the variables
            setMock(bypassTokensCount(configId), 0);
